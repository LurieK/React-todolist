...
const [joytodo, setJoyTodo] = useState('');
  ...
  return (
    ...
    <div className='work'>
      <h3>Work To-Dos</h3>
      <form className='work' onSubmit={(e) => handleSubmit(e, 'WorkToDo', worktodo, setWorkTodo)}>
      <input type="text" onChange={(e) => setWorkTodo(e.target.value)} value={worktodo} />
      ...
      {todos.WorkToDo.map((workItem)=> 
        ...
        <input
            ...
            />
          <div className= {workItem.completed ? 'completed' : 'work-text'}>{workItem.text}</div>
       </div>)}
        </div>
        
       <button className= 'clear' onClick={()=> clearToDo('WorkToDo')}>Clear!</button>
      </div> 

      <div className= 'life'>
      <h3>Self Care To-Dos</h3>
      <form className='life' onSubmit={(e) => handleSubmit(e, 'LifeToDo', lifetodo, setLifeTodo)}>
      <input type="text" onChange={(e) => setLifeTodo(e.target.value)} value={lifetodo} />
        <button type='submit'>Enter</button>
      </form>
      ...
      />
        <div className={lifeItem.completed ? 'completed' : 'work-text'}>{lifeItem.text}</div>
        </div>)}
        </div>

      <button className= 'clear' onClick={()=> clearToDo('LifeToDo')}>Clear!</button>
      </div>

      <div className='joy'>
        <h3>Pleasure To-Dos</h3>
      <form className='joy' onSubmit={(e) => handleSubmit(e, 'JoyToDo', joytodo, setJoyTodo)}>
      <input type="text" onChange={(e) => setJoyTodo(e.target.value)} value={joytodo} />
        <button type='submit'>Enter</button>
      </form>
      ...
      {todos.JoyToDo.map((joyItem)=> 
        ...
        <input
            ...
            />
          <div className= {joyItem.completed ? 'completed' : 'work-text'}>{joyItem.text}</div>
        </div>)}
        </div>
        
        <button className= 'clear' onClick= {()=> clearToDo('JoyToDo')}>Clear!</button>
        </div>
        </div>
    </div>
  );
